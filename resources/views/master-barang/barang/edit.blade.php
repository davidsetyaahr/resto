        ...
        <div class="card">
            ...
            <form action="{{ route('barang.update', $barang->kode_barang) }}" method="post">
              ...
              <div class="card-body">
                  ...
                  <input type="text" name="kode_barang" value="{{old('kode_barang', $barang->kode_barang)}}" class="form-control @error('kode_barang') is-invalid @enderror" placeholder="ex : BR0001">
                  @error('kode_barang')
                  ...
                  <input type="text" name="nama" value="{{old('nama', $barang->nama)}}" class="form-control @error('nama') is-invalid @enderror" placeholder="ex : Beras">
                  @error('nama')
                    <span class="invalid-feedback" role="alert">
                      <strong>{{ $message }}</strong>
                    </span>
                  ...
                  <select name="id_kategori_barang" class="form-control select2 @error('id_kategori_barang') is-invalid @enderror" id="">
                    ...
                    @foreach ($kategoris as $item)
                      <option value="{{$item->id_kategori_barang}}" {{old('id_kategori_barang', $barang->id_kategori_barang) == $item->id_kategori_barang ? 'selected' : ''}} > {{$item->kategori_barang}} </option>
                    @endforeach
                  </select>
                  ...
                  <input type="text" name="satuan" value="{{old('satuan', $barang->satuan)}}" class="form-control @error('satuan') is-invalid @enderror" placeholder="ex : Kg">
                  @error('satuan')
                    <span class="invalid-feedback" role="alert">
                      <strong>{{ $message }}</strong>
                    </span>
                  ...
                  <input type="number" name="minimum_stock" value="{{old('minimum_stock', $barang->minimum_stock)}}" class="form-control @error('minimum_stock') is-invalid @enderror" placeholder="ex : 25">
                  @error('minimum_stock')
                    <span class="invalid-feedback" role="alert">
                      <strong>{{ $message }}</strong>
                    </span>
                  ...
                  <div class="input-group mb-3">
                    ...
                    <input type="text" name="exp_date" value="{{old('exp_date', $barang->exp_date)}}" class="form-control datepicker @error('exp_date') is-invalid @enderror" placeholder="ex : 2024-08-08">
                  </div> 
                  @error('exp_date')
                    <span class="invalid-feedback" role="alert">
                      <strong>{{ $message }}</strong>
